...
public class ComponentTwo implements IObserver {
    ...
    public ComponentTwo() {
        String file = "/music/sample-15s.wav";
        try {
            InputStream audioSrc = getClass().getResourceAsStream(file);
            if (audioSrc == null) {
                throw new IllegalArgumentException("Music is not found: " + file);
            }
    ...
    public void update(Subject subject) {
        ...
        if (timeServer.isRunning()) {
            if (musicStartTime == -1) {
                ...
                System.out.println("The music start time is set to: " + musicStartTime);
            }
